...
def generate_makeflows(pointsCsv, mdlScn, prjScns, numPerGroup):
    ...
    with open(pointsCsv) as inF:
        ...
        for c in chunks:
            ...
            log_files = ["{}.makeflowlog".format(chunkFn), "{}.wqlog".format(chunkFn), "{}.wqlog.tr".format(chunkFn)]
            master['rules'].append({
                "command": "makeflow -l {}.makeflowlog -L {}.wqlog --jx-context makeflows/params.jx --jx-context {} makeflows/taxa.jx ".format(chunkFn, chunkFn, chunkFn, chunkFn) + ' '.join(["&& touch {}".format(x) for x in log_files]),
